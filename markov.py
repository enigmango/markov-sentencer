...
Notes: Written with Python 2.7.6. Should work with mac, linux, windows. Run from command line.
May break with Python 3.0 and higher due to "raw_input()" method being changed in python 3.
'''
...
def pickSeed():
	...
	#seed = '"go \'way!" "Ain\'t' # debug
	# print "Seed = " + " " + pSeed # debug
	return pSeed
...
def makeMarkov(mSeed):
	...
	while numSentences > countSentences:
		...
		if string.lower(lastWord) not in honorifics and (lastWord[-1] in sentenceEndings or lastWord[-2:] in quoteEndings):
			countSentences += 1	
...
def textCleanup(markovIn, custom):
	...
	output = string.join(output) # done!
...
if sys.argv[1][0] == "*": # custom specified
	uSeed = raw_input("Seed: ")
	numSentences = raw_input("Number of sentences: ")
	prefLen = raw_input("Prefix length (higher is more accurate to the text): ") # second last is the prefix length
	listOut = raw_input("Print dictionary? ")
